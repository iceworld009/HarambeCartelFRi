...
@Autonomous(name = "Auto Close Blue Bad ", group = "Test")
public class AutoCloseBlueBad extends OpMode {
    ...
    public void stopPresiune() {
        PoseStorage.autoPose = follower.getPose();
        selectioner.unloadBallsSlow();
        motors.setRampVelocityC((int) (0.45 * targetVelocity));
        sleep(100);
        motors.intakeOff();
        hold(0);
    }
    ...
    double getHood(double d) {
        if (d < 130) {
            ...
            return 3 + (d - 170) * (1.6 / 50.0);
        } else {
            return 4.6 + (d - 220) * (2.4 / 130.0);
        }
    }
