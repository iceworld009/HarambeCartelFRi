...
public enum Geometry {
    ;
    public static double getDistanceOD(double x, double y, int color){ //Formula automata pentru cele doua cosuri
        double distance = 0;
        if(color == 0){//red
    ...
    public static double getDistanceODMan(double x, double y, double targetX, double targetY){
        return Math.sqrt(Math.pow(targetX-x,2)+Math.pow(targetY-y,2)) * 2.54;
    }

    public static double getRobotAngle(double x, double y, int target){
        if(target == 0)
