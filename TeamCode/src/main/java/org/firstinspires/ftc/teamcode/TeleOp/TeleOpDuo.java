...
@TeleOp(name="TeleOp Duo", group = "Solo")
public class TeleOpDuo extends LinearOpMode {
    ...
    boolean turretOn = false, intakeOn = false;
    ...
    double voltage;
    Pose BotPose;
    double feedforwardboost = 0;
    ElapsedTime check = new ElapsedTime();
    ElapsedTime command = new ElapsedTime();
    ...
    public void runOpMode()  {
        ...
        while(opModeIsActive()) {
            ...
            } else if (motors.getVelocity() < 1800 && turretOn && !rampUp && selectioner.ballsfull) {
                motors.setRampVelocityC(1800);
            }
            ...
            if (gamepad1.right_stick_button || gamepad2.b) {
                target = 0;
                lastTarget = 0;
                limelight.setPipeline(4);
                limelight.stop();
            } else if (gamepad1.left_stick_button || gamepad2.x) {
                target = 1;
                lastTarget = 1;
            ...
            else if (selectioner.ballsfull) {
                motors.intakeSetVelocity(300);
            }
            else {
                motors.intakeOff();
            }
            ...
            if (gamepad1.right_bumper) {
                ...
                turretOn = true;
                gamepad1.rumble(300);
                motors.setRampCoefs(voltage,feedforwardboost);
            ...
            if((gamepad1.dpad_left || gamepad2.a)&& command.milliseconds() > 200){
                feedforwardboost-=0.2;
                command.reset();
                gamepad2.rumble(75);
            }

            if((gamepad1.dpad_right || gamepad2.y)&& command.milliseconds() > 200){
                feedforwardboost+=0.2;
                command.reset();
                gamepad2.rumble(75);
            }
    ...
    int getRPM(double d) {
        ...
        if(distance>280)r+=450;
        return (int) Math.max(1000, Math.min(r, 4300));
    }
    ...
    void updateTelemetry(){
        ...
        telemetry.addData("Feedforwardboost:",feedforwardboost);
        telemetry.addData("IntakeRpm:",motors.getIntakeVelocity());
        telemetry.update();
    }
}